...
.DESCRIPTION
    Downloads TeamViewer QS LITS to temp directory and executes
.LINK
    https://everydayintech.com
#>

param (
    [string]$DownloadUrl = "https://customdesignservice.teamviewer.com/download/windows/v15/3cjghq6/TeamViewerQS.exe",
    [string]$FileName = "TeamViewerQS.exe",
    [string]$SavePath = (Get-Item $env:TEMP).FullName,
    [bool]$ExpansionNeeded = $false,
    [string]$ExpandDirName = '',
    [string]$ExecutablePath = 'TeamViewerQS.exe'
)
