<#
.SYNOPSIS
Opens IntuneManagementExtension.log with CMTrace.exe
.DESCRIPTION
...
function Invoke-CMTraceFileOpen {
    [CmdletBinding()]
    param (
        [Parameter()][string]$Path = "$($env:ProgramData)\Microsoft\IntuneManagementExtension\Logs\IntuneManagementExtension.log",
        [Parameter()][string]$CMTrace = "$($env:TEMP)\CMTrace.exe"
    )

    Invoke-Expression "& $CMTrace $Path"
...
Invoke-CMTraceFileOpen `
    -Path "$($env:ProgramData)\Microsoft\IntuneManagementExtension\Logs\IntuneManagementExtension.log" `
    -CMTrace $CMTrace.FullName
